...
client
  ...
  .on('message', async (message) => {
    ...
    message.channel.send(gameInit).then(async (msg) => {
      ...
      await msg.react(reaction_[3]);
      await msg.react(reaction_[4]);
      await msg.react(reaction_[5]);
      await msg.react(reaction_[6]);
      await msg.react(reaction_[7]);
      await msg.edit(GameThread.newGame());
    });
  ...
  .on('message', (message) => {
    if (message.author.id === client.user.id) {
      ...
      if (message.channel.id === dumpChannel) {
        games.nextTurn((message.attachments).array()[0].url, message.content);
      }
    }
  ...
  .on('messageReactionAdd', async (reaction, user) => {
    ...
    switch (reaction.emoji.id) {
      ...
      case reaction_[7]:
        ...
        break;
      default:
      // needed default case because of linter supremacy
    }
  })// Logs in the client
  .login(token);
